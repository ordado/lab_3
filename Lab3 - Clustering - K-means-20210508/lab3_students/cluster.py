...
class Cluster:
    ...
    def compute_centroid(self):
        ...
        cordinate_x_all_points = []
        cordinate_y_all_points = []
        cordinate_z_all_points = []
        for point in self._points:
            for coordinate_index, coordinate_value in enumerate(point.coordinates):
                if coordinate_index == 0:
                    cordinate_x_all_points.append(coordinate_value)
                else:
                    if coordinate_index == 1:
                        cordinate_y_all_points.append(coordinate_value)
                    else:
                        cordinate_z_all_points.append(coordinate_value)

        cordinate_x_all_points.sort()
        cordinate_y_all_points.sort()
        cordinate_z_all_points.sort()

        len_of_point = len(cordinate_x_all_points)
        if len_of_point % 2 == 0:
            len_of_point = int(len_of_point / 2) - 1
            new_coordinates[0] = (cordinate_x_all_points[len_of_point] + cordinate_x_all_points[len_of_point + 1]) / 2
            new_coordinates[1] = (cordinate_y_all_points[len_of_point] + cordinate_y_all_points[len_of_point + 1]) / 2
            new_coordinates[2] = (cordinate_z_all_points[len_of_point] + cordinate_z_all_points[len_of_point + 1]) / 2
        else:
            new_coordinates[0] = cordinate_x_all_points[math.ceil((len_of_point - 1) / 2)]
            new_coordinates[1] = cordinate_y_all_points[math.ceil((len_of_point - 1) / 2)]
            new_coordinates[2] = cordinate_z_all_points[math.ceil((len_of_point - 1) / 2)]
